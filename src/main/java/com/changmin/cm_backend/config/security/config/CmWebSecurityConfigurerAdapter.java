...
import com.changmin.cm_backend.config.security.core.filter.TokenAuthenticationFilter;
...
public class CmWebSecurityConfigurerAdapter {
   ...
   * 配置 URL 的安全配置
   *
   * <p>anyRequest | 匹配所有请求路径 access | SpringEl表达式结果为true时可以访问 anonymous | 匿名可以访问 denyAll | 用户不能访问
  ...
  protected SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
    ...
    httpSecurity
        ...
        .permitAll()
        .antMatchers(HttpMethod.PATCH, permitAllUrls.get(HttpMethod.PATCH).toArray(new String[0]))
        .permitAll()
        .antMatchers(HttpMethod.DELETE, permitAllUrls.get(HttpMethod.DELETE).toArray(new String[0]))
        .permitAll()
        // 1.3 基于 changmin.security.permit-all-urls 无需认证
